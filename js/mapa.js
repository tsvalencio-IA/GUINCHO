(function () {
  ...
  function nextRenderToken(containerId) {
    renderTokens[containerId] = (renderTokens[containerId] || 0) + 1;
    return renderTokens[containerId];
  }

  function isRenderStale(containerId, token, map) {
    const container = document.getElementById(containerId);
    const stale = renderTokens[containerId] !== token || !container || !container.isConnected || (map && map._container !== container);
    if (stale && map) {
      try { map.remove(); } catch (_) {}
      if (liveMaps[containerId] === map) {
  ...
  function containerCanRender(container) {
    if (!container || !container.isConnected) return false;
    const style = window.getComputedStyle ? window.getComputedStyle(container) : null;
    if (style && (style.display === "none" || style.visibility === "hidden")) return false;
    const rect = container.getBoundingClientRect ? container.getBoundingClientRect() : { width: container.offsetWidth, height: container.offsetHeight };
    return Number(rect.width || container.offsetWidth || 0) > 0 && Number(rect.height || container.offsetHeight || 0) > 0;
  }
  ...
  function resetMap(containerId, container) {
    if (liveMaps[containerId]) {
      try { liveMaps[containerId].off(); } catch (_) {}
      try { liveMaps[containerId].remove(); } catch (_) {}
      delete liveMaps[containerId];
      delete tileLayers[containerId];
    }
    if (container && container._leaflet_id) {
      try { delete container._leaflet_id; } catch (_) { try { container._leaflet_id = null; } catch (__) {} }
    }
  }
  ...
  async function renderFleetMap(containerId, vehicles, calls, options) {
    ...
    const token = nextRenderToken(containerId);
    const container = document.getElementById(containerId);
    if (!container) return;
    if (!containerCanRender(container)) {
      if ((options.deferCount || 0) < 4) {
        setTimeout(() => {
          if (renderTokens[containerId] === token) renderFleetMap(containerId, vehicles, calls, Object.assign({}, options, { deferCount: (options.deferCount || 0) + 1 }));
        }, 180);
      }
      return;
    }
    ...
    try {
      ...
      if (isRenderStale(containerId, token)) return;
      resetMap(containerId, container);
      container.innerHTML = "";
      const map = L.map(container, { scrollWheelZoom: false, preferCanvas: true, zoomControl: false });
      ...
      for (const { call, pts } of routedCalls) {
        ...
        if (pts.length >= 2) {
          await addRouteLayer(L, map, call, pts, bounds);
          if (isRenderStale(containerId, token, map)) return;
        }
      }
      if (isRenderStale(containerId, token, map)) return;
      ...
      setTimeout(() => {
        ...
        if (selectedMarker) {
          try { selectedMarker.openPopup(); } catch (_) {}
        }
      }, 120);
    } catch (err) {
      console.warn(err);
  ...
  function invalidateAll() {
    Object.entries(liveMaps).forEach(([containerId, map]) => {
      const container = map && map._container || document.getElementById(containerId);
      if (!containerCanRender(container)) return;
      try { map.invalidateSize(); } catch (_) {}
    });
  }
