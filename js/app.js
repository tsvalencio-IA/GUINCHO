(function () {
  ...
  const LOGIN_FLOW_VERSION = "jm-fluxo-gestor-v32-cache-leaflet-hotfix";
...
Rota: ${url}`;
  ...
  function aiReviewedCallPayload(index, draft) {
    ...
    reviewed.normalizedCall = {
      ...
      parserVersion: "jm-fluxo-gestor-v32-cache-leaflet-hotfix"
    };
  ...
  async function createOfficialCallFromAi(index) {
    ...
    const callData = compactForFirestore({
      ...
      aiReviewed: true,
      aiCreatedAt: now,
      aiParserVersion: "jm-fluxo-gestor-v32-cache-leaflet-hotfix",
  ...
  window.JM = window.JM || {};
  // Compatibilidade: alguns botões/HTML legado e auditorias antigas procuram estas funções direto no window.
  // O fluxo principal continua usando JM.app.*, sem trocar a lógica operacional existente.
  window.selectOperationalCall = selectOperationalCall;
  window.selectCallDossier = selectCallDossier;
  window.JM.app = {
    setCallStatus,
