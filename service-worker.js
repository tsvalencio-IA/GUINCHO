const CACHE_NAME = "jm-fluxo-global-v32-cache-control";
const CURRENT_VERSION = "jm-fluxo-global-v32-cache-control";
const GESTOR_VERSION = "jm-fluxo-gestor-v32-cache-leaflet-hotfix";
const MOTORISTA_VERSION = "jm-fluxo-operacional-v31-motorista-cloudinary-real-flow";
const ACTIVE_ASSET_VERSIONS = new Set([CURRENT_VERSION, GESTOR_VERSION, MOTORISTA_VERSION]);
...
async function purgeOldVersionEntries() {
  ...
  await Promise.all(requests.map((request) => {
    ...
    if (OLD_VERSION_RE.test(url)) return cache.delete(request);
...
function shouldCacheRequest(request) {
  ...
  if (version && !ACTIVE_ASSET_VERSIONS.has(version)) return false;
  if (OLD_VERSION_RE.test(url.href)) return false;
  return true;
}
